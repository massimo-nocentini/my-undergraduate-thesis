...
tale che $\rightarrow \subseteq V \times V$.

Un grafo $G$ si dice non orientato se $\rightarrow$ \`e una relazione
...
tali che $\forall k \in [i, j-1]$ si ha $(v_{k},v_{k+1}) \in
\rightarrow$. Tale cammino lo indicheremo con la scrittura $v_{i}
\rightarrow^{*} v_{j}$. Nel caso di un grafo non orientato indichiamo
un cammino con la scrittura $v_{i} -^{*} v_{j}$.
...
propriet\`a:
\begin{itemize}
\item la versione \emph{non orientata} di $T$ \`e connessa;
\item esiste esattamente un vertice $v$ tale che $\forall w \in V: (w,
  v) \not \in \rightarrow$. Il vertice $v$ viene chiamato \emph{root}
  di $T$;
...
tale che $G_{i} = (V_{i}, E_{i})$, con $E_{i} = \{(r,s) \in
\rightarrow: r,s \in V_{i} \}$. Allora:
\begin{itemize}
